...
static void        uri_resolved                      (YelpUri            *uri,
                                                      YelpView           *view);
static void        yelp_view_register_custom_schemes ();
...
static void        view_load_status_changed          (WebKitWebView            *view,
                                                      WebKitLoadEvent           load_event,
                                                      gpointer                  user_data);
static void        yelp_view_register_extensions     ();
                            ...
                            gpointer             user_data)
{
    ...
    if (webkit_hit_test_result_context_is_image (hit_test_result) ||
        ...
        if (g_str_has_prefix (yelp_uri, "help:") ||
            ...
            if (image && image[0] != '\0')
                priv->popup_image_uri = yelp_uri_locate_file_uri (priv->uri, image);
            else
                priv->popup_image_uri = NULL;
        }
        else {
